...
async function readPosts() {
  ...
  const posts = await store.get(KEYS.posts, { type: "json", consistency: "strong" });
...
async function readInquiries() {
  const store = getStoreClient();
  const inquiries = await store.get(KEYS.inquiries, { type: "json", consistency: "strong" });
...
async function readNewsletter() {
  const store = getStoreClient();
  const list = await store.get(KEYS.newsletter, { type: "json", consistency: "strong" });
...
async function readSiteContent() {
  const store = getStoreClient();
  const raw = await store.get(KEYS.siteContent, { type: "json", consistency: "strong" });
